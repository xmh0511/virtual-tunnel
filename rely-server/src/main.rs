...
async fn main() {
    ...
    let data_cryptor = Aes256Cryptor::try_from(&config.encrypt_key).unwrap();
        ...
        loop {
            ...
            match rx.recv().await {
                ...
                Some(Message::Data((_num, buff))) => {
                    let cleartext_buff = descrypt_bytes(buff.clone(), &data_cryptor);
                    if cleartext_buff.len() < 20 {
                        tracing::info!(
                            "Receive an invalid packet from peer, len = {}",
                            cleartext_buff.len()
                        );
                        continue;
                    }
                    //println!("receive data from {_num}:\n{cleartext_buff:?}");
                    if let Some((source, dest)) = ip_dest_parse::get_dest_ip(&cleartext_buff) {
                        if let Some(writer) = find_another(&map, dest.clone()).await {
                            ...
                            tokio::spawn(async move {
                                ...
                                match write_all::write_all(&mut writer, buff).await {
                                    ...
                                    Err(e) => {
                                        ...
                                        if let Some(tx) = self_sender_weak.upgrade() {
    ...
    while let Ok((mut stream, socket_addr)) = listen.accept().await {
        ...
        let vir_addr = match read_node_list().get(&index) {
            ...
            None => {
                ...
                tracing::info!(
                    r#"The identifier "{index}" does not exist in the group, shutdown the connection"#
                );
